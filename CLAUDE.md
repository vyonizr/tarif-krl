...
- `src/app/types.ts` holds MRT interfaces and shared UI state types (`IStationState`, etc). KRL request/response/domain types live in `src/lib/krl/types.ts`, the one exception to "no per-feature type files," since it outgrew the shared file.
