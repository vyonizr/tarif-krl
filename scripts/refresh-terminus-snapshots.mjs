...
// station's full-day schedule straight from KCI, plus the full stop
// sequence for every train seen in those schedules, commits any changes
// into data/schedule-snapshots/ and data/train-snapshots/, and pushes that
// commit directly to main so the bundled fallbacks
// (src/lib/krl/snapshotStore.ts's getRepoScheduleSnapshot and
// getRepoTrainScheduleSnapshot) redeploy with fresh data. Run with
...
async function fetchTrainSchedule(trainId) {
  const url =
    `${KCI_BASE_URL}/train-schedule?` + new URLSearchParams({ trainid: trainId })
  const res = await fetch(url, { signal: AbortSignal.timeout(10_000) })
  if (!res.ok) throw new Error(`HTTP ${res.status}`)
  const json = await res.json()
  return json.data
...
async function main() {
  ...
  for (const stationId of TERMINUS_STATIONS) {
    ...
    try {
      ...
      changedStations.push(stationId)
...
async function refreshTrainSnapshots(trainIds) {
  ...
  for (let i = 0; i < ids.length; i += TRAIN_FETCH_CONCURRENCY) {
    const batch = ids.slice(i, i + TRAIN_FETCH_CONCURRENCY)
